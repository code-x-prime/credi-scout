...
export default function WhoWeArePage() { return <><Header/><main>
  <PeopleHero/>
  <section className="section compact-section"><div className="container split-section">
  ...
  <section className="section section-muted compact-section"><div className="container"><SectionHeading eyebrow="Security, quality & data protection" title="Security, quality & data protection standards" body="Banking verification involves sensitive customer, financial, and business information. CrediScout places information security, process quality, and responsible data handling at the core of its verification operations and technology."/><div className="standards-grid">{standards.map(([a,b])=><article className="standard-card reveal" key={a}><img src={standardIcons[a]} alt={a} width={40} height={40} className="standard-icon"/><h3>{a}</h3><p>{b}</p></article>)}</div></div></section>
  ...
  <section className="section section-muted compact-section"><div className="container"><SectionHeading eyebrow="What guides us" title="Our core values"/><div className="values-list">{coreValues.map(([a,b])=><article className="value-item reveal" key={a}><h3>{a}</h3><p>{b}</p></article>)}</div></div></section>
